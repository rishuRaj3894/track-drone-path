...
export class AppComponent implements OnDestroy {
  ...
  async onSimulate() {
    ...
    this.time = this.latLngForm.value.time ? (this.latLngForm.value.time * 1000) / 100 : 0
    ...
    if (this.isFileUpload) {
      ...
      console.log(this.fileContent.length, 'this.fileContent.length ')
      for (let i = 0; i <= this.fileContent.length - 2; i++) {
        console.log(this.fileContent[i], i, "this.fileContent[i] this.fileContent[i]")
        this.time = this.fileContent[i].timestamp
        await this.updateMarker(this.fileContent[i].lat, this.fileContent[i].lng, 2, false, 'E')
        this.positionA = {
          lat: this.fileContent[i].lat,
          lng: this.fileContent[i].lng
        }
        this.positionB = {
          lat: this.fileContent[i + 1].lat,
          lng: this.fileContent[i + 1].lng
        }
        await this.startDroneUpdates(this.positionA, this.positionB);
      }
    } else {
      this.updateMarker(this.positionB.lat, this.positionB.lng, 2, false, 'E')
      this.startDroneUpdates(this.positionA, this.positionB);
    }


  }

  startDroneUpdates(positionA?: any, positionB?: any) {
    this.droneUpdateSubscription = interval(this.time).subscribe(() => {
      if (this.progressCount <= this.numSteps) {
        ...
        const intermediateLat = positionA.lat + fraction * (positionB.lat - positionA.lat);
        const intermediateLng = positionA.lng + fraction * (positionB.lng - positionA.lng);
    ...
    if (input.files && input.files.length) {
      ...
      reader.onload = (e) => {
        try {
          ...
          console.log('Parsed JSON:', this.fileContent);
          this.startMarker = this.polyVertices[0];
          this.endMarker = this.polyVertices[this.polyVertices.length - 1];
        } catch (err) {
